 ...
 * @public
 */
export function enableOverlays(): DisableVisualEditing {
  if (root || node) return cleanup;
  let cancelled = false;

  // Lazy load everything needed to render the app
  Promise.all([import('react-dom/client'), import('./components/VisualEditingOverlay')]).then(
    ([reactClient, { VisualEditingOverlay }]) => {
      if (cancelled) return;
